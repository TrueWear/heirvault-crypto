...
import { unlockVaultCryptoV2, parseVaultCrypto, type VaultCryptoV2 } from './vault'

/**
 * A real VaultCryptoV2 blob, committed once and never regenerated, so this
 ...
 * the same test run, so a change that broke both `createVaultCryptoV2` and
 * `unlockVaultCryptoV2` identically (a real risk: they share
 ...
 * and the fixed salt 0x00..0x0f via a live run of createVaultCryptoV2, then
...
describe('vault crypto golden fixture', () => {
  it('parses a committed VaultCryptoV2 blob from a prior run', () => {
    const parsed: VaultCryptoV2 = parseVaultCrypto(GOLDEN_VAULT_CRYPTO_JSON)
  ...
  it('unwraps the committed blob to the exact DEK it was created with', async () => {
    ...
    const { dek } = await unlockVaultCryptoV2(GOLDEN_PASSPHRASE, vaultCrypto)
  ...
  it('rejects the committed blob under the wrong passphrase', async () => {
    const vaultCrypto = parseVaultCrypto(GOLDEN_VAULT_CRYPTO_JSON)
    await expect(
      unlockVaultCryptoV2('not-the-golden-passphrase', vaultCrypto)
    ).rejects.toThrow()
  }, 90_000)
})
